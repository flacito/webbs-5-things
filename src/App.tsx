...
import { Menu, X, Sun, Moon, Monitor, ChevronRight, Link as LinkIcon } from 'lucide-react';
  ...
  return (
    <nav className="toc">
      {onToggleVisible ? (
        ...
        <h3 className="text-lg font-semibold text-gh-heading mb-4 px-3">Contents</h3>
      )}
      <div
        className={`transition-all duration-300 ease-in-out ${
          isVisible === false ? 'max-h-0 opacity-0 overflow-hidden' : 'max-h-[60vh] opacity-100 overflow-y-auto'
        }`}
      >
...
function HeadingWithAnchor({ level, children, id, ...props }: HeadingProps & { level: number }) {
  ...
  switch (level) {
    case 1: return <h1 id={id} className="group" {...props}>{headingContent}</h1>;
    case 2: return <h2 id={id} className="group" {...props}>{headingContent}</h2>;
    case 3: return <h3 id={id} className="group" {...props}>{headingContent}</h3>;
    case 4: return <h4 id={id} className="group" {...props}>{headingContent}</h4>;
    default: return <h2 id={id} className="group" {...props}>{headingContent}</h2>;
  }
}
...
function App() {
  ...
  const [autoExpandedSection, setAutoExpandedSection] = useState<string | null>(null);
  ...
  const getParentSectionId = useCallback((headingId: string): string | null => {
    ...
    <div className="min-h-screen bg-gh-bg">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-gh-bg-secondary border-b border-gh-border">
        <div className="flex items-center justify-between px-4 py-3">
          <h1 className="text-lg font-semibold text-gh-heading">Webb's 5 Things</h1>
      ...
      <div className="lg:flex max-w-7xl mx-auto">
        ...
        <main className="flex-1 pt-[57px] lg:pt-0 lg:pl-4">
          ...
          <footer className="border-t border-gh-border py-8 mt-16 text-center text-sm text-gh-text/60">
            ...
            <p className="mt-2">
              <a href="mailto:brian@kingslandacademy.com" className="text-gh-link hover:underline">
